...
\begin{tabular}{ll}
    $\bm{A}$ & is a matrix \\
     $\bm{A}^{\top}$ & is a matrix transpose \\
     $\bm{A}^{-1}$ & is a matrix inverse \\
     $\bm{a}$ & is a vector \\
     $a$ & is a scalar \\
...
\bm{S} = \frac{1}{n-1} \bm{X}^{\top} \bm{X}
\end{equation}

and is therefore size $(Q \times Q)$. Notice the similarity with the eq.(\ref{eq:covariance-definition}).

We will start by exploring the meaning of $\bm{X}^{\top} \bm{X}$. Let's look at the graphical representation of this matrix multiplication in Fig.~\ref{fig:covariance-matrix}.

Any given column, say $p$ or $k$, of the data matrix $\bm{X}$ represents all measurements of a single $p^{th}$ (or $k^{th}$) variable and can be viewed as an $n$-dimensional vector. The same can be said about any row of a matrix $\bm{X}^{\top}$.
...
Notice that an element at position $(p,k)$ inside the covariance matrix has the interpretation of a dot product between a vector formed by the $p$-th row of a matrix $\bm{X}^{\top}$ and the $k$-th column of a matrix $\bm{X}$.
%There is also the factor $\frac{1}{n-1}$ out front, which we discuss in a closer detail in the Appendix \ref{app:B}.

\begin{equation}
s_{p,k} = \frac{1}{n-1} (\bm{x}_p^{\top} \circ \bm{x}_k)
\end{equation}
...
\begin{equation}
s_{p,p} = \frac{1}{n-1} ( \bm{x}_p^{\top} \circ \bm{x}_p)
\end{equation}
...
First, any matrix $\bm{S}$ constructed as $\bm{S} = \bm{C}^{\top} \bm{C}$ (where $\bm{C}$ is any real matrix) is square and symmetric. The reason for the symmetry can be seen looking at Fig.~\ref{fig:covariance-matrix} -- notice that the element $(p,k)$ has the same value as the element $(k,p)$ since it is a result of multiplying the same two vectors, just in different order. Notice also that there is a symmetry in: $\text{cov}(\bm{x},\bm{y}) = \text{cov}(\bm{y},\bm{x})$. 

Second, the eigenvalues of $\bm{S}$ are real and all are at least non-negative. Such matrix is called a \textit{positive semidefinite} matrix. In a practical case that we are most interested in, this matrix has got only positive eigenvalues and we call it a \textit{positive definite} matrix.
...
$\bm{A}$ is the matrix of eigenvectors and it is size $(Q \times Q)$. Each eigenvector defines the direction for the \textit{principal components} (PCs). The principal components are orthogonal (due to properties discussed in \ref{sec:covariance-properties}) and therefore another important property holds: $\bm{A}^{\top} = \bm{A}^{-1}$ (proof\footnote{for orthogonal columns of $\bm{A}$ we have $\bm{A}^{\top} \bm{A} = \bm{I}$. Multiplying both sides from the right by $ \bm{A}^{-1}$ we get $(\bm{A}^{\top} \bm{A}) \bm{A}^{-1}= \bm{I}\bm{A}^{-1}$. Since matrix multiplication is associative, we may also perform: $\bm{A}^{\top} (\bm{A} \bm{A}^{-1}) = \bm{A}^{-1}$. From the definition of an inverse matrix $\bm{A} \bm{A}^{-1} = \bm{I}$. Hence $\bm{A}^{\top} = \bm{A}^{-1}$.}).
...
Given the eigendecomposition of the matrix $\bm{S}$, we may state that: 

\begin{equation} \label{eq:eig-dec-cov-matrix}
\bm{S} = \bm{A} \bm{\Lambda} \bm{A}^{\top}
\end{equation}
...
The above equation is our route back to obtain the original data set in which the PC-scores are projected on the basis associated with a transposed eigenvectors matrix $\bm{A}$ (the fact that $\bm{A}^{\top} = \bm{A}^{-1}$ now appears in all  of its usefulness).
...
Projecting $\bm{Z_q}$ onto the basis $\bm{A_q}^{\top}$ will result in an an approximation of the original data set:

\begin{equation} \label{eq:X-approximation}
\bm{X_q} = \bm{Z_q} \bm{A_q}^{\top}
\end{equation}

It is now worth discussing how good is such approximation. It turns out that $\bm{X_q}$ is the closest rank-$q$ approximation to $\bm{X}$ in terms of three popular norms: $L^2$-norm, Frobenius norm and trace norm. This is known as the Eckart-Young theorem \cite{eckart-young}.
...
\begin{equation}
\bm{S_Z} =  \frac{1}{n-1} \bm{Z}^{\top} \bm{Z}
\end{equation}
...
\begin{equation}
\bm{A} \bm{\Lambda} \bm{A}^{\top} =  \frac{1}{n-1} \bm{X}^{\top} \bm{X} \,\, \Big/ \bm{A}^{\top} \times
\end{equation}

\begin{equation}
\bm{A}^{\top} \bm{A} \bm{\Lambda} \bm{A}^{\top} =  \frac{1}{n-1} \bm{A}^{\top}  \bm{X}^{\top} \bm{X} \,\, \Big/ \times \bm{A}
\end{equation}

\begin{equation}
\bm{A}^{\top} \bm{A} \bm{\Lambda} \bm{A}^{\top} \bm{A} =  \frac{1}{n-1} \bm{A}^{\top}  \bm{X}^{\top} \bm{X} \bm{A}
\end{equation}

\begin{equation}
\bm{I} \bm{\Lambda} \bm{I} =  \frac{1}{n-1} \bm{A}^{\top}  \bm{X}^{\top} \bm{X} \bm{A}
\end{equation}

\begin{equation}
\bm{S_Z}  = \bm{\Lambda} = \frac{1}{n-1} \bm{A}^{\top}  \bm{X}^{\top} \bm{X} \bm{A}
\end{equation}

It is thus visible, that if we chose $\bm{Z} = \bm{X} \bm{A}$, the product $\bm{Z}^{\top} \bm{Z}$ give a diagonal matrix.\footnote{Note here that if $\bm{Z} = \bm{X} \bm{A}$, then $\bm{Z}^{\top} = (\bm{X} \bm{A})^{\top} = \bm{A}^{\top} \bm{X}^{\top}$.}
...
Hopefully you could now see for yourself that the basis associated with the eigenvectors of the covariance matrix is a very interesting basis! It lets us diagonalize the product $\bm{Z}^{\top} \bm{Z}$ and it orders the PC-scores from the most to the least \textit{informative} ones.
...
%
%But what about the approximation of the data $\bm{X_q} = \bm{Z_q} \bm{A_q}^{\top}$ when $q=Q$? What is the rank of the reconstructed $\bm{X_Q}$?
%
%
...
\bm{D}_{\text{app}} = \text{PC-scores}(:,1:q) \cdot \text{PCs}^{\top}(1:q,:) + \bm{D}_{\text{mean}}
\end{equation}
