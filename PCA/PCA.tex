...
These are notes on \textbf{Principal Component Analysis} (PCA), a dimensionality reduction technique in which a large (high-dimensional) data set can be reduced to maintain only the directions of the largest variance. The technique is used broadly in many branches of science such as astrophysics, psychology or fluid dynamics, to name a few. As many other data science techniques, PCA allows not only to make a low-rank approximation of the data set, but also to extract and observe additional information hidden in the data, which would otherwise be difficult or impossible task for a human.

\,\,

These notes are in a way a tutorial on PCA with a deeper focus on linear algebra and statistics aspects governing this method. I have the aim that the reader will both gain a deep and intuitive understanding as well as an appreciation for PCA. This requires both a deep and intuitive understanding of several linear algebra concepts such as the eigendecomposition or matrix operations. 
...
We can now answer the previously posed question: the basis associated with the eigenvectors of the covariance matrix is a very interesting basis! It lets us diagonalize the product $\bm{Z}^T \bm{Z}$ and it orders the PC-scores from the most to the least "informative" ones.
...
\subsection{The intuition behind $\frac{1}{1-n}$.}
