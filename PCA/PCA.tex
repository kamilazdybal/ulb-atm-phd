...
These are notes on \textbf{Principal Component Analysis} (PCA), a dimensionality reduction technique in which a large, high-dimensional data set is reduced to maintain only the directions (dimensions) of the largest variance. It bases on the fact that the original basis to represent the data set might not be an \textit{optimal} one. In PCA, a new basis is found and the data is transformed to that new basis giving certain advantages. Similarly as in some other data science techniques, PCA allows not only to make a low-rank approximation of the data set, but also to extract and observe additional information hidden in the data.
...
Some initial level of linear algebra knowledge is required. I highly recommend a very pleasant to watch course by 3Blue1Brown YouTube channel \cite{3Blue1Brown}, which, to my belief, will be everything you need to master about linear algebra to build a great intuition behind PCA.
